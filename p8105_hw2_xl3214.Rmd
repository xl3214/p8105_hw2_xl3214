...
```{r import and clean the dataset of baseline demographics, message = FALSE}
...
```{r import and clean the dataset of longitudinally observed biomarker values, message = FALSE}
...
When importing the dataset, we can notice that the default header of the uncleaned dataset are *`r colnames(mci_amyloid_description)`*, which are actually descriptions of each variable. Printing the dataset out, we would notice that the actual header is actually the second row: *`r colnames(mci_amyloid)`*. To tidy the dataset, I used `pivot_longer` function to change several columns of different times of followup to rows. 

There are `r count(subset(mci_baseline, !id %in% mci_amyloid$id))` participants who appeared in only the baseline dataset but not the amyloid dataset, and `r count(subset(mci_amyloid, !id %in% mci_baseline$id))` participants who appeared in only the amyloid dataset not the baseline dataset. To ensure that only participants with entries in both dataset are included into the final dataset, I will merge the two datasets using the `inner_join` function. 
...
There are `r nrow(data_q3)` observations and `r ncol(data_q3)` variables in the final merged dataset for MCI. Variables include *`r colnames(data_q3)`*. An Aβ42/40 ratio <0.150 suggests a higher risk of having of AD pathology compared to higher values. 2 years after baseline, `r (count(filter(data_q3, amyroid_ratio < 0.15 & years_elapsed_since_baseline == 2))/count(filter(data_q3, years_elapsed_since_baseline == 2)))*100`% of participants have An Aβ42/40 ratio <0.150. 4 years after baseline, the proportion increased to `r (count(filter(data_q3, amyroid_ratio < 0.15 & years_elapsed_since_baseline == 4))/count(filter(data_q3, years_elapsed_since_baseline == 4)))*100`%. 6 years after baseline, the proportion increased to `r (count(filter(data_q3, amyroid_ratio < 0.15 & years_elapsed_since_baseline == 6))/count(filter(data_q3, years_elapsed_since_baseline == 6)))*100`%, and 8 years after baseline, the proportion increased to `r (count(filter(data_q3, amyroid_ratio < 0.15 & years_elapsed_since_baseline == 8))/count(filter(data_q3, years_elapsed_since_baseline == 8)))*100`%. Among those that are female, the proportion of having an Aβ42/40 ratio <0.150 8 years after baseline is `r (count(filter(data_q3, amyroid_ratio < 0.15 & years_elapsed_since_baseline == 8 & sex == "Female"))/count(filter(data_q3, years_elapsed_since_baseline == 8 & sex == "Female")))*100`%, and the proportion is `r (count(filter(data_q3, amyroid_ratio < 0.15 & years_elapsed_since_baseline == 8 & sex == "Male"))/count(filter(data_q3, years_elapsed_since_baseline == 8 & sex == "Male")))*100`% for males. 
